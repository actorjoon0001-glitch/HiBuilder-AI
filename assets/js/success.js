// 주문 완료 페이지 - 토스에서 돌아오면 paymentKey/orderId/amount 를 서버로 보내 승인 확정
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (paymentKey && orderId && amount) {
    ...
    try {
      ...
      if (!resp.ok || data.status !== 'paid') {
        return renderError(root, product, data.error || '결제 승인에 실패했습니다.');
      }
      if (typeof window.track === 'function') {
        window.track('Purchase', {
          ...
          items: product ? [{ id: product.id, name: product.title, price: product.price }] : []
        });
      }
      renderSuccess(root, product, { orderId, amount, method: data.method });
    } catch (err) {
      console.error(err);
      renderError(root, product, '결제 승인 중 통신 오류가 발생했습니다.');
    }
    return;
  }
  ...
  renderSuccess(root, product, { orderId: order?.id, amount: order?.price || product?.price });
});

function renderPending(root, product) {
  root.innerHTML = `
    <div class="container">
      <div class="success-box">
        ...
        ${product ? `<p class="muted" style="margin-top:10px">${product.title}</p>` : ''}
      </div>
    </div>
  `;
}

function renderSuccess(root, product, info) {
  root.innerHTML = `
    <div class="container">
      <div class="success-box">
        <div class="check">✓</div>
        <h1 style="font-size:24px;font-weight:800;margin:0 0 8px">주문이 완료되었습니다</h1>
        <p class="muted" style="margin:0 0 22px">주문내역을 이메일로 보내드렸어요. 배송·시공 일정은 담당자가 안내드립니다.</p>
        ${product ? `
          <div style="text-align:left;padding:16px;border:1px solid var(--line);border-radius:10px;background:var(--bg);margin-bottom:22px">
            ...
            <div style="margin-top:6px;color:var(--ink-2)">
              ${info?.amount ? fmtPrice(Number(info.amount)) : fmtPrice(product.price)}
              ${info?.method ? ` · ${info.method}` : ''}
            </div>
          </div>
        ` : ''}
        <a href="index.html" class="btn btn-primary btn-lg">계속 쇼핑하기</a>
        <a href="partners.html" class="btn btn-ghost btn-lg" style="margin-top:8px">시공 상담 신청하기</a>
      </div>
    </div>
  `;
}
  ...
  root.innerHTML = `
    <div class="container">
      <div class="success-box">
        ...
        <p class="muted" style="margin:0 0 8px">${escapeHtml(message)}</p>
        ...
        <a href="checkout.html?id=${product ? product.id : ''}" class="btn btn-primary btn-lg">다시 시도</a>
